...
def fftd(img, backwards=False, byRow=False):
    # DFT_INVERSE: 执行反向一维或二维转换，而不是默认的正向转换
    # DFT_SCALE: 表示缩放结果，由阵列元素的数量除以它
    # DFT_ROWS:执行正向或反向变换输入矩阵的每个单独的行，该标志可以同时转换多个矢量，并可用于减少开销以执行3D和更高维度的转换
    # DFT_COMPLEX_OUTPUT: 对一维或二维的实数数组进行正向变换，这样的结果虽然是复数阵列，但拥有复数的共轭对称性

    if byRow:
        # cv2.dft(原始图像，转换标识),原始图像必须是np.float32格式
...
"""
    对于一张图片进行傅里叶变换就是将它分解为正弦余弦两部分，
    以完成从空间域到频率域的转换，转换到频域时
    以复数形式存在，因此变换后的结果需要使用实数图像和虚数图像
"""
...
def complexMultiplication(a, b):  # 复数乘法
    #  np.zeros(形状, 数据类型)
    res = np.zeros(a.shape, a.dtype)

    res[:, :, 0] = a[:, :, 0] * b[:, :, 0] - a[:, :, 1] * b[:, :, 1]  # 实部
    res[:, :, 1] = a[:, :, 0] * b[:, :, 1] + a[:, :, 1] * b[:, :, 0]  # 虚部
    return res


# 两个复数，它们相除 (a+bi)/(c+di)=(ac+bd)/(c*c+d*d) +((bc-ad)/(c*c+d*d))i
def complexDivision(a, b):  # 复数除法
    res = np.zeros(a.shape, a.dtype)
    divisor = 1. / (b[:, :, 0] ** 2 + b[:, :, 1] ** 2)

    res[:, :, 0] = (a[:, :, 0] * b[:, :, 0] + a[:, :, 1] * b[:, :, 1]) * divisor  # 实部
    res[:, :, 1] = (a[:, :, 1] * b[:, :, 0] + a[:, :, 0] * b[:, :, 1]) * divisor  # 虚部
    return res
...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multi_scale=False):
        self.lambdar = 0.0001  # 正则化
        self.padding = 2.5  # 目标扩展出来的区域
        ...
        if multi_scale:
            ...
            if hog == False:
                print('HOG feature is forced to turn on.')  # Hog功能被强制打开

        elif fixed_window:
            self.template_size = 96  # 模板大小
            self.scale_step = 1  # 尺度步长
        else:
        ...
        self.scaleFactors = None  # 所有尺度变化率，从大到小，以1为中间
        self.scale_model_width = 0  # 用于缩放的模型宽度
        self.scale_model_height = 0  # 用于缩放的模型高度
    ...
    def gaussianCorrelation(self, x1, x2):
        if self._hogfeatures:
            ...
            c = cv2.mulSpectrums(fftd(x1), fftd(x2), 0, conjB=True)  # 计算两个频谱的逐元素乘积
            c = fftd(c, True)
            c = real(c)
            c = rearrange(c)
    ...
    def detect(self, z, x):
        ...
        # cv2.resize(res, (640, 480))
        # cv2.imshow('hanning', res)
        # cv2.waitKey(0)
