...
def fftd(img, backwards=False, byRow=False):
    ...
    # byRow=False 按列输入，byRow=True 按行输入
...
def rearrange(img):
    ...
    xh, yh = img.shape[1] // 2, img.shape[0] // 2 # shape[0] 为行，shape[1] 为列  //地板除
    #将两个斜对角的数据交换
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
    ...
    if (border != [0, 0, 0, 0]):
        res = cv2.copyMakeBorder(res, border[1], border[3], border[0], border[2], borderType,value=(255,255,255))
    return res
...
class KCFTracker:
    ...
    def createGaussianPeak(self, sizey, sizex):
        ...
        #ogrid函数产生的数组，第一个是纵向即列向量，第二个是横向即行向量
    ...
    def init(self, roi, image):
        # map将roi中的每个项处理为float型  list将元组转换为列表（元组元素值不可修改）
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.):
        ...
        cx = self._roi[0] + self._roi[2] / 2  #找中心点
    ...
    def detect(self, z, x):
        ...
        # 得到响应最大点索引的float表示
        p = [float(pi[0]), float(pi[1])]  # 响应最大值坐标
